...
# The trajectory array is assigned before the process pool is created. On a
# Linux system, forked workers inherit it using copy-on-write, so the complete
# array is not deliberately copied once for every lag time.
_worker_trajectories = None


def ngp_worker(lag_time_index):
    """Calculate the simulated NGP for one lag-time index."""
    return non_gaussian_parameter(
        None, _worker_trajectories, lag_time_index
    )
...
if __name__ == "__main__":
    ...
    for idx, H in enumerate(tqdm(H_list, desc="Processing H values")):
        ...
        # Each lag time is independent, so calculate the simulation points in
        # parallel. The number of workers never exceeds the number of lag-time
        # points or the number of CPUs available on the machine.
        _worker_trajectories = trajectories
        n_workers = min(
            20, len(lag_times_index), os.cpu_count() or 1
        )
        print(
            f"Computing simulated NGP using {n_workers} CPU processes..."
        )
        ...
        with ProcessPoolExecutor(
            ...
            ngp_vals = list(
                tqdm(
                    executor.map(ngp_worker, lag_times_index),
                    total=len(lag_times_index),
                    desc=f"NGP H={H}",
                    leave=False,
                )
            )
        ngp_vals = np.asarray(ngp_vals)
